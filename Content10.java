        ...
        while (true) {
            ...
            if (command.equals("삭제")) {
                ...
                if (file.exists()) {
                    ...
                    File[] files = dir.listFiles((d, name) -> name.endsWith(".json") && name.matches("\\d+\\.json"));
            ...
            if (command.equals("빌드")) {
                File[] files = dir.listFiles((d, name) -> name.matches("\\d+\\.json"));
                if (files == null || files.length == 0) {
                    System.out.println("빌드할 명언이 없음.");
                    continue;
                }


                try (FileWriter fw = new FileWriter(DB_PATH + "/data.json")) {
                    fw.write("[\n");

                    for (int i = 0; i < files.length; i++) {
                        int lid = Integer.parseInt(files[i].getName().replace(".json", ""));
                        Map<String, String> data = loadWiseSaying(lid);

                        fw.write("  {\n");
                        fw.write("    \"id\": " + lid + ",\n");
                        fw.write("    \"auther\": \"" + data.get("auther") + "\",\n");
                        fw.write("    \"content\": \"" + data.get("content") + "\"\n");
                        if (i == files.length - 1)
                            fw.write("  }\n");
                        else
                            fw.write("  },\n");
